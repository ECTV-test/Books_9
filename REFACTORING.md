...
## Статус: Этап 1.6 — Блок 8 выполнен, финальная чистка app.js (в процессе)
...
| До | После | Разница |
|---|---|---|
| 6069 строк | ~3470 строки | **−2600 строк (−43%)** |
| 1 файл | 11 модулей | логическая структура |
...
### ✅ Блок 8 — Reader views → `js/views/reader.js` (~295 строк)

**`js/views/reader.js`** — `renderReader` + `renderBiReader`
- Обе функции вынесены в отдельный файл
- Зависимости задокументированы в шапке файла
- `index.html` обновлён: добавлен `<script src="js/views/reader.js?v=1"></script>`
- **Из `app.js` нужно удалить** оригинальные `renderReader()` и `renderBiReader()` (~280 строк)
- После удаления: app.js ~3470 строк (было 3754)
...
```
index.html          ← store.js убран; reader.js подключён (v=1); app.js v=7
css/
  styles.css
...
js/
  ...
  views/
    ...
    reader.js       ← renderReader + renderBiReader (~295 строк) ✅
  app.js            ← 3754 строки → после удаления дублей ~3470
```
...
<script src="js/views/reader.js?v=1"></script>
...
1. **Удалить дубли из app.js** — вырезать `renderReader()` и `renderBiReader()` (~280 строк). Заменить на комментарий `/* ── renderReader, renderBiReader → js/views/reader.js ── */`
2. **UI фиксы** (низкий риск) — 4 задачи выше в разделе "Открытые задачи"
