...
function handlePlayerBack(){
  ...
  if(r === "reader" || r === "bireader"){
    const bookId = resolveBookId();
    if(bookId){ go({name:"details", bookId}, {push:false}); return; }
  }
  return appBack();
}
        ...
        ${list.length ? list.map((it,i)=>`
          <div class="bmSheetItem">
            ...
            <div class="bmSheetActions">
              <button class="bmBtn" data-bm-play="${escapeHtml(bookId)}::${escapeHtml(it.id)}" title="Play">${typeof _SVG_BM_SPEAK!=="undefined"?_SVG_BM_SPEAK:"🔊"}</button>
              <button class="bmBtn primary" data-bm-go="${escapeHtml(bookId)}::${escapeHtml(it.id)}" title="Go">${typeof _SVG_BM_GO!=="undefined"?_SVG_BM_GO:"↪︎"}</button>
              <button class="bmBtn" data-bm-del="${escapeHtml(bookId)}::${escapeHtml(it.id)}" title="Delete">${typeof _SVG_BM_DEL!=="undefined"?_SVG_BM_DEL:"✕"}</button>
            </div>
          </div>
